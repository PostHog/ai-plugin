...
FROM
    (SELECT
        ...
        least(19, intDiv(dateDiff('seconds', toDateTime(toDateTime('2026-09-03 11:00:00.000000')), timestamp), greatest(1, intDiv(dateDiff('seconds', toDateTime(toDateTime('2026-09-03 11:00:00.000000')), toDateTime(toDateTime('2026-09-04 11:32:54.561578'))), 20)))) AS bin_idx,
        count() AS occ,
    ...
    FROM
        events AS e
    WHERE
        and(equals(e.event, '$exception'), isNotNull(e.properties.$exception_fingerprint), true, greaterOrEquals(e.timestamp, toDateTime(toDateTime('2026-09-03 11:00:00.000000'))), lessOrEquals(e.timestamp, toDateTime(toDateTime('2026-09-04 11:32:54.561578'))), or(greater(position(lower(e.properties.$exception_types), lower('constant')), 0), greater(position(lower(e.properties.$exception_values), lower('constant')), 0), greater(position(lower(e.properties.$exception_sources), lower('constant')), 0), greater(position(lower(e.properties.$exception_functions), lower('constant')), 0), greater(position(lower(e.properties.email), lower('constant')), 0), greater(position(lower(e.person.properties.email), lower('constant')), 0)), equals(properties.tag, 'max_ai'))
    GROUP BY
        fp_hash,
        bin_idx) AS ev
